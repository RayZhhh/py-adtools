...
class VLLMServer:
    def __init__(
            ...
            host: str = '0.0.0.0',
            mem_util: float = 0.85,
            deploy_timeout_seconds: int = 360,
...
class MultiServerScheduler:
    def __init__(self, vllm_servers: List[VLLMServer]):
        self._vllm_servers = vllm_servers

    def close(self):
        for vllm_server in self._vllm_servers:
            vllm_server.close()

    def _sequential_strategy(self) -> int:
        pass

    def _random_strategy(self) -> int:
        return random.randint(0, len(self._vllm_servers) - 1)

    def _lower_usage_strategy(self) -> int:
        pass
